---
name: researcher
description: Open-web research with explicit WebSearch and WebFetch — answers a question from sources it actually fetched, quoting each one. Use when the answer is not in the repo but on the web — docs, upstream changelogs, prices, availability, current facts.
model: opus
effort: medium
